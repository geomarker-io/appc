...
d %>%
  summarize(
    ...
    rsq = cor.test(conc, pred, method = "spearman", exact = FALSE)$estimate,
...
add_mean_CIs_and_summarize_error <- function(x) {
  x |>
    ...
    summarize(
      ...
      rsq = cor.test(mean_pm25, mean_pm_pred, method = "spearman", exact = FALSE)$estimate,
...
d_temporal_cv$time <-
  d_temporal_cv$time |>
  factor(levels = c("annual", "monthly", "weekly", "daily"))
...
d_spatiotemporal_cv <-
  d_temporal_cv |>
  mutate(s2_ = s2_cell_parent(s2, 4)) |>
  group_by(time, s2_) |>
  summarise(
