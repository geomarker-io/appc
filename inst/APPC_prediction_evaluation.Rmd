...
grf <- readRDS(fs::path(tools::R_user_dir("appc", "data"), glue::glue("rf_pm_v{packageVersion('appc')}.rds")))
d_train <- readRDS(fs::path(tools::R_user_dir("appc", "data"), glue::glue("training_data_v{packageVersion('appc')}.rds")))
```
...
tibble(importance = round(variable_importance(grf), 3),
  ...
  mutate(predictors = list(data$variable)) |>
  select(-data) |>
  mutate(predictors = list(paste(predictors, collapse = ", "))) |>
